...
$(document).ready(function () {
    ...
    iLotus.changeTheme = {
        A: function () {
            var theme = this.check();
            if (theme == 'day') {
                $('#J-html').removeClass('iLight');
                jQuery.cookie('iTheme', 'night', {expires: 7, path: '/'});
                $("#J-changeTheme i").attr('class', "icon-moon");
            } else {
                $('#J-html').addClass('iLight');
                jQuery.cookie('iTheme', 'day', {expires: 7, path: '/'});
                $("#J-changeTheme i").attr('class', "icon-sun");
            }
        },
        B: function () {
            if (this.check() == "day") {
